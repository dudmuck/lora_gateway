...
static struct lgw_tx_gain_lut_s txgain_lut[2] = {
    {
        .size = 2,
        .lut[0] = {
            .dig_gain = 0,
...
int load_firmware(uint8_t, uint8_t target, uint8_t *firmware, uint16_t size);

void lgw_constant_adjust(uint8_t);
...
int load_firmware(uint8_t csn, uint8_t target, uint8_t *firmware, uint16_t size)
{
    int reg_rst;
    int reg_sel;
    ...
    lgw_reg_w(csn, reg_rst, 1);

    /* set mux to access MCU program RAM and set address to 0 */
    lgw_reg_w(csn, reg_sel, 0);
    lgw_reg_w(csn, LGW_MCU_PROM_ADDR, 0);

    /* write the program in one burst */
    lgw_reg_wb(csn, LGW_MCU_PROM_DATA, firmware, size);

    /* Read back firmware code for check */
    lgw_reg_r(csn,  LGW_MCU_PROM_DATA, &dummy ); /* bug workaround */
    lgw_reg_rb(csn,  LGW_MCU_PROM_DATA, fw_check, size );
...
void lgw_constant_adjust(uint8_t csn)
{
    /* I/Q path setup */
    // lgw_reg_w(csn, LGW_RX_INVERT_IQ,0); /* default 0 */
    // lgw_reg_w(csn, LGW_MODEM_INVERT_IQ,1); /* default 1 */
    // lgw_reg_w(csn, LGW_CHIRP_INVERT_RX,1); /* default 1 */
    // lgw_reg_w(csn, LGW_RX_EDGE_SELECT,0); /* default 0 */
    // lgw_reg_w(csn, LGW_MBWSSF_MODEM_INVERT_IQ,0); /* default 0 */
    // lgw_reg_w(csn, LGW_DC_NOTCH_EN,1); /* default 1 */
    lgw_reg_w(csn, LGW_RSSI_BB_FILTER_ALPHA,6); /* default 7 */
    lgw_reg_w(csn, LGW_RSSI_DEC_FILTER_ALPHA,7); /* default 5 */
    lgw_reg_w(csn, LGW_RSSI_CHANN_FILTER_ALPHA,7); /* default 8 */
    lgw_reg_w(csn, LGW_RSSI_BB_DEFAULT_VALUE,23); /* default 32 */
    lgw_reg_w(csn, LGW_RSSI_CHANN_DEFAULT_VALUE,85); /* default 100 */
    lgw_reg_w(csn, LGW_RSSI_DEC_DEFAULT_VALUE,66); /* default 100 */
    lgw_reg_w(csn, LGW_DEC_GAIN_OFFSET,7); /* default 8 */
    lgw_reg_w(csn, LGW_CHAN_GAIN_OFFSET,6); /* default 7 */

    /* Correlator setup */
    // lgw_reg_w(csn, LGW_CORR_DETECT_EN,126); /* default 126 */
    // lgw_reg_w(csn, LGW_CORR_NUM_SAME_PEAK,4); /* default 4 */
    // lgw_reg_w(csn, LGW_CORR_MAC_GAIN,5); /* default 5 */
    // lgw_reg_w(csn, LGW_CORR_SAME_PEAKS_OPTION_SF6,0); /* default 0 */
    // lgw_reg_w(csn, LGW_CORR_SAME_PEAKS_OPTION_SF7,1); /* default 1 */
    // lgw_reg_w(csn, LGW_CORR_SAME_PEAKS_OPTION_SF8,1); /* default 1 */
    // lgw_reg_w(csn, LGW_CORR_SAME_PEAKS_OPTION_SF9,1); /* default 1 */
    // lgw_reg_w(csn, LGW_CORR_SAME_PEAKS_OPTION_SF10,1); /* default 1 */
    // lgw_reg_w(csn, LGW_CORR_SAME_PEAKS_OPTION_SF11,1); /* default 1 */
    // lgw_reg_w(csn, LGW_CORR_SAME_PEAKS_OPTION_SF12,1); /* default 1 */
    // lgw_reg_w(csn, LGW_CORR_SIG_NOISE_RATIO_SF6,4); /* default 4 */
    // lgw_reg_w(csn, LGW_CORR_SIG_NOISE_RATIO_SF7,4); /* default 4 */
    // lgw_reg_w(csn, LGW_CORR_SIG_NOISE_RATIO_SF8,4); /* default 4 */
    // lgw_reg_w(csn, LGW_CORR_SIG_NOISE_RATIO_SF9,4); /* default 4 */
    // lgw_reg_w(csn, LGW_CORR_SIG_NOISE_RATIO_SF10,4); /* default 4 */
    // lgw_reg_w(csn, LGW_CORR_SIG_NOISE_RATIO_SF11,4); /* default 4 */
    // lgw_reg_w(csn, LGW_CORR_SIG_NOISE_RATIO_SF12,4); /* default 4 */

    /* LoRa 'multi' demodulators setup */
    // lgw_reg_w(csn, LGW_PREAMBLE_SYMB1_NB,10); /* default 10 */
    // lgw_reg_w(csn, LGW_FREQ_TO_TIME_INVERT,29); /* default 29 */
    // lgw_reg_w(csn, LGW_FRAME_SYNCH_GAIN,1); /* default 1 */
    // lgw_reg_w(csn, LGW_SYNCH_DETECT_TH,1); /* default 1 */
    // lgw_reg_w(csn, LGW_ZERO_PAD,0); /* default 0 */
    lgw_reg_w(csn, LGW_SNR_AVG_CST,3); /* default 2 */
    if (lorawan_public) { /* LoRa network */
        lgw_reg_w(csn, LGW_FRAME_SYNCH_PEAK1_POS,3); /* default 1 */
        lgw_reg_w(csn, LGW_FRAME_SYNCH_PEAK2_POS,4); /* default 2 */
    } else { /* private network */
        lgw_reg_w(csn, LGW_FRAME_SYNCH_PEAK1_POS,1); /* default 1 */
        lgw_reg_w(csn, LGW_FRAME_SYNCH_PEAK2_POS,2); /* default 2 */
    }

    // lgw_reg_w(csn, LGW_PREAMBLE_FINE_TIMING_GAIN,1); /* default 1 */
    // lgw_reg_w(csn, LGW_ONLY_CRC_EN,1); /* default 1 */
    // lgw_reg_w(csn, LGW_PAYLOAD_FINE_TIMING_GAIN,2); /* default 2 */
    // lgw_reg_w(csn, LGW_TRACKING_INTEGRAL,0); /* default 0 */
    // lgw_reg_w(csn, LGW_ADJUST_MODEM_START_OFFSET_RDX8,0); /* default 0 */
    // lgw_reg_w(csn, LGW_ADJUST_MODEM_START_OFFSET_SF12_RDX4,4092); /* default 4092 */
    // lgw_reg_w(csn, LGW_MAX_PAYLOAD_LEN,255); /* default 255 */

    /* LoRa standalone 'MBWSSF' demodulator setup */
    // lgw_reg_w(csn, LGW_MBWSSF_PREAMBLE_SYMB1_NB,10); /* default 10 */
    // lgw_reg_w(csn, LGW_MBWSSF_FREQ_TO_TIME_INVERT,29); /* default 29 */
    // lgw_reg_w(csn, LGW_MBWSSF_FRAME_SYNCH_GAIN,1); /* default 1 */
    // lgw_reg_w(csn, LGW_MBWSSF_SYNCH_DETECT_TH,1); /* default 1 */
    // lgw_reg_w(csn, LGW_MBWSSF_ZERO_PAD,0); /* default 0 */
    if (lorawan_public) { /* LoRa network */
        lgw_reg_w(csn, LGW_MBWSSF_FRAME_SYNCH_PEAK1_POS,3); /* default 1 */
        lgw_reg_w(csn, LGW_MBWSSF_FRAME_SYNCH_PEAK2_POS,4); /* default 2 */
    } else {
        lgw_reg_w(csn, LGW_MBWSSF_FRAME_SYNCH_PEAK1_POS,1); /* default 1 */
        lgw_reg_w(csn, LGW_MBWSSF_FRAME_SYNCH_PEAK2_POS,2); /* default 2 */
    }
    // lgw_reg_w(csn, LGW_MBWSSF_ONLY_CRC_EN,1); /* default 1 */
    // lgw_reg_w(csn, LGW_MBWSSF_PAYLOAD_FINE_TIMING_GAIN,2); /* default 2 */
    // lgw_reg_w(csn, LGW_MBWSSF_PREAMBLE_FINE_TIMING_GAIN,1); /* default 1 */
    // lgw_reg_w(csn, LGW_MBWSSF_TRACKING_INTEGRAL,0); /* default 0 */
    // lgw_reg_w(csn, LGW_MBWSSF_AGC_FREEZE_ON_DETECT,1); /* default 1 */

    /* FSK datapath setup */
    lgw_reg_w(csn, LGW_FSK_RX_INVERT,1); /* default 0 */
    lgw_reg_w(csn, LGW_FSK_MODEM_INVERT_IQ,1); /* default 0 */

    /* FSK demodulator setup */
    lgw_reg_w(csn, LGW_FSK_RSSI_LENGTH,4); /* default 0 */
    lgw_reg_w(csn, LGW_FSK_PKT_MODE,1); /* variable length, default 0 */
    lgw_reg_w(csn, LGW_FSK_CRC_EN,1); /* default 0 */
    lgw_reg_w(csn, LGW_FSK_DCFREE_ENC,2); /* default 0 */
    // lgw_reg_w(csn, LGW_FSK_CRC_IBM,0); /* default 0 */
    lgw_reg_w(csn, LGW_FSK_ERROR_OSR_TOL,10); /* default 0 */
    lgw_reg_w(csn, LGW_FSK_PKT_LENGTH,255); /* max packet length in variable length mode */
    // lgw_reg_w(csn, LGW_FSK_NODE_ADRS,0); /* default 0 */
    // lgw_reg_w(csn, LGW_FSK_BROADCAST,0); /* default 0 */
    // lgw_reg_w(csn, LGW_FSK_AUTO_AFC_ON,0); /* default 0 */
    lgw_reg_w(csn, LGW_FSK_PATTERN_TIMEOUT_CFG,128); /* sync timeout (allow 8 bytes preamble + 8 bytes sync word, default 0 */

    /* TX general parameters */
    lgw_reg_w(csn, LGW_TX_START_DELAY, TX_START_DELAY); /* default 0 */

    /* TX LoRa */
    // lgw_reg_w(csn, LGW_TX_MODE,0); /* default 0 */
    lgw_reg_w(csn, LGW_TX_SWAP_IQ,1); /* "normal" polarity; default 0 */
    if (lorawan_public) { /* LoRa network */
        lgw_reg_w(csn, LGW_TX_FRAME_SYNCH_PEAK1_POS,3); /* default 1 */
        lgw_reg_w(csn, LGW_TX_FRAME_SYNCH_PEAK2_POS,4); /* default 2 */
    } else { /* Private network */
        lgw_reg_w(csn, LGW_TX_FRAME_SYNCH_PEAK1_POS,1); /* default 1 */
        lgw_reg_w(csn, LGW_TX_FRAME_SYNCH_PEAK2_POS,2); /* default 2 */
    }

    /* TX FSK */
    // lgw_reg_w(csn, LGW_FSK_TX_GAUSSIAN_EN,1); /* default 1 */
    lgw_reg_w(csn, LGW_FSK_TX_GAUSSIAN_SELECT_BT,2); /* Gaussian filter always on TX, default 0 */
    // lgw_reg_w(csn, LGW_FSK_TX_PATTERN_EN,1); /* default 1 */
    // lgw_reg_w(csn, LGW_FSK_TX_PREAMBLE_SEQ,0); /* default 0 */
...
int lgw_rxrf_setconf(uint8_t csn, uint8_t rf_chain, struct lgw_conf_rxrf_s conf)
{
    ...
    /* set internal config according to parameters */
    gw[csn].rf_enable[rf_chain] = conf.enable;
    gw[csn].rf_rx_freq[rf_chain] = conf.freq_hz;
    gw[csn].rf_rssi_offset[rf_chain] = conf.rssi_offset;
    gw[csn].rf_radio_type[rf_chain] = conf.type;
    gw[csn].rf_tx_enable[rf_chain] = conf.tx_enable;
    gw[csn].rf_tx_notch_freq[rf_chain] = conf.tx_notch_freq;

    DEBUG_PRINTF("Note: rf_chain %d configuration; en:%d freq:%d rssi_offset:%f radio_type:%d tx_enable:%d tx_notch_freq:%u\n", rf_chain, gw[csn].rf_enable[rf_chain], gw[csn].rf_rx_freq[rf_chain], gw[csn].rf_rssi_offset[rf_chain], gw[csn].rf_radio_type[rf_chain], gw[csn].rf_tx_enable[rf_chain], gw[csn].rf_tx_notch_freq[rf_chain]);
...
int lgw_rxif_setconf(uint8_t csn, uint8_t if_chain, struct lgw_conf_rxif_s conf)
{
    ...
    if (conf.enable == false) {
        gw[csn].if_enable[if_chain] = false;
        gw[csn].if_freq[if_chain] = 0;
    ...
    switch (ifmod_config[if_chain]) {
        case IF_LORA_STD:
            ...
            gw[csn].if_enable[if_chain] = conf.enable;
            gw[csn].if_rf_chain[if_chain] = conf.rf_chain;
            gw[csn].if_freq[if_chain] = conf.freq_hz;
            gw[csn].lora_rx_bw = conf.bandwidth;
            gw[csn].lora_rx_sf = (uint8_t)(DR_LORA_MULTI & conf.datarate); /* filter SF out of the 7-12 range */
            if (SET_PPM_ON(conf.bandwidth, conf.datarate)) {
                gw[csn].lora_rx_ppm_offset = true;
            } else {
                gw[csn].lora_rx_ppm_offset = false;
            }

            DEBUG_PRINTF("Note: LoRa 'std' if_chain %d configuration; en:%d freq:%d bw:%d dr:%d\n", if_chain, gw[csn].if_enable[if_chain], gw[csn].if_freq[if_chain], gw[csn].lora_rx_bw, gw[csn].lora_rx_sf);
            break;
        ...
        case IF_LORA_MULTI:
            ...
            gw[csn].if_rf_chain[if_chain] = conf.rf_chain;
            gw[csn].if_freq[if_chain] = conf.freq_hz;
            gw[csn].lora_multi_sfmask[if_chain] = (uint8_t)(DR_LORA_MULTI & conf.datarate); /* filter SF out of the 7-12 range */

            DEBUG_PRINTF("Note: LoRa 'multi' if_chain %d configuration; en:%d freq:%d SF_mask:0x%02x\n", if_chain, gw[csn].if_enable[if_chain], if_freq[if_chain], gw[csn].lora_multi_sfmask[if_chain]);
            break;
        ...
        case IF_FSK_STD:
            ...
            gw[csn].if_rf_chain[if_chain] = conf.rf_chain;
            gw[csn].if_freq[if_chain] = conf.freq_hz;
            gw[csn].fsk_rx_bw = conf.bandwidth;
            gw[csn].fsk_rx_dr = conf.datarate;
            if (conf.sync_word > 0) {
                fsk_sync_word_size = conf.sync_word_size;
                fsk_sync_word = conf.sync_word;
            }
            DEBUG_PRINTF("Note: FSK if_chain %d configuration; en:%d freq:%d bw:%d dr:%d (%d real dr) sync:0x%0*llX\n", if_chain, gw[csn].if_enable[if_chain], gw[csn].if_freq[if_chain], gw[csn].fsk_rx_bw, gw[csn].fsk_rx_dr, LGW_XTAL_FREQU/(LGW_XTAL_FREQU/gw[csn].fsk_rx_dr), 2*fsk_sync_word_size, fsk_sync_word);
            break;
...
int lgw_txgain_setconf(uint8_t csn, struct lgw_tx_gain_lut_s *conf)
{
    ...
    txgain_lut[csn].size = conf->size;

    for (i = 0; i < txgain_lut[csn].size; i++) {
        /* Check gain range */
        ...
        /* Set internal LUT */
        txgain_lut[csn].lut[i].dig_gain = conf->lut[i].dig_gain;
        txgain_lut[csn].lut[i].dac_gain = conf->lut[i].dac_gain;
        txgain_lut[csn].lut[i].mix_gain = conf->lut[i].mix_gain;
        txgain_lut[csn].lut[i].pa_gain  = conf->lut[i].pa_gain;
        txgain_lut[csn].lut[i].rf_power = conf->lut[i].rf_power;
    }
...
int lgw_start(uint8_t csn)
{
    ...
    reg_stat = lgw_connect(csn, false, gw[csn].rf_tx_notch_freq[gw[csn].rf_tx_enable[1]?1:0]);
    ...
    /* gate clocks */
    lgw_reg_w(csn, LGW_GLOBAL_EN, 0);
    lgw_reg_w(csn, LGW_CLK32M_EN, 0);

    /* switch on and reset the radios (also starts the 32 MHz XTAL) */
    lgw_reg_w(csn, LGW_RADIO_A_EN,1);
    lgw_reg_w(csn, LGW_RADIO_B_EN,1);
    wait_ms(500); /* TODO: optimize */
    lgw_reg_w(csn, LGW_RADIO_RST,1);
    wait_ms(5);
    lgw_reg_w(csn, LGW_RADIO_RST,0);

    /* setup the radios */
    err = lgw_setup_sx125x(csn, 0, rf_clkout, gw[csn].rf_enable[0], gw[csn].rf_radio_type[0], gw[csn].rf_rx_freq[0]);
    if (err != 0) {
        fprintf(stderr,"ERROR: Failed to setup sx125x radio for RF chain 0\n");
        return LGW_HAL_ERROR;
    }
    err = lgw_setup_sx125x(csn, 1, rf_clkout, gw[csn].rf_enable[1], gw[csn].rf_radio_type[1], gw[csn].rf_rx_freq[1]);
    if (err != 0) {
        fprintf(stderr,"ERROR: Failed to setup sx125x radio for RF chain 0\n");
        return LGW_HAL_ERROR;
    }
    ...
    lgw_reg_w(csn, LGW_GPIO_MODE,31); /* Set all GPIOs as output */
    lgw_reg_w(csn, LGW_GPIO_SELECT_OUTPUT,2);

#ifndef DISABLE_FPGA
    /* Configure LBT */
    if (lbt_is_enabled() == true) {
        lgw_reg_w(csn, LGW_CLK32M_EN, 1);
        i = lbt_setup(csn);
    ...
    */
    ...
    cal_cmd = 0;
    cal_cmd |= gw[csn].rf_enable[0] ? 0x01 : 0x00; /* Bit 0: Calibrate Rx IQ mismatch compensation on radio A */
    cal_cmd |= gw[csn].rf_enable[1] ? 0x02 : 0x00; /* Bit 1: Calibrate Rx IQ mismatch compensation on radio B */
    cal_cmd |= (gw[csn].rf_enable[0] && gw[csn].rf_tx_enable[0]) ? 0x04 : 0x00; /* Bit 2: Calibrate Tx DC offset on radio A */
    cal_cmd |= (gw[csn].rf_enable[1] && gw[csn].rf_tx_enable[1]) ? 0x08 : 0x00; /* Bit 3: Calibrate Tx DC offset on radio B */
    cal_cmd |= 0x10; /* Bit 4: 0: calibrate with DAC gain=2, 1: with DAC gain=3 (use 3) */

    switch (gw[csn].rf_radio_type[0]) { /* we assume that there is only one radio type on the board */
    ...
    load_firmware(csn, MCU_AGC, cal_firmware, MCU_AGC_FW_BYTE);
    lgw_reg_w(csn, LGW_FORCE_HOST_RADIO_CTRL, 0); /* gives to AGC MCU the control of the radios */
    lgw_reg_w(csn, LGW_RADIO_SELECT, cal_cmd); /* send calibration configuration word */
    lgw_reg_w(csn, LGW_MCU_RST_1, 0);

    /* Check firmware version */
    lgw_reg_w(csn, LGW_DBG_AGC_MCU_RAM_ADDR, FW_VERSION_ADDR);
    lgw_reg_r(csn, LGW_DBG_AGC_MCU_RAM_DATA, &read_val);
    ...
    lgw_reg_w(csn, LGW_PAGE_REG, 3); /* Calibration will start on this condition as soon as MCU can talk to concentrator registers */
    lgw_reg_w(csn, LGW_EMERGENCY_FORCE_HOST_CTRL, 0); /* Give control of concentrator registers to MCU */

    /* Wait for calibration to end */
    DEBUG_PRINTF("Note: calibration started (time: %u ms)\n", cal_time);
    wait_ms(cal_time); /* Wait for end of calibration */
    lgw_reg_w(csn, LGW_EMERGENCY_FORCE_HOST_CTRL, 1); /* Take back control */

    /* Get calibration status */
    lgw_reg_r(csn, LGW_MCU_AGC_STATUS, &read_val);
    ...
    */
    ...
    if (gw[csn].rf_enable[0] && ((cal_status & 0x02) == 0)) {
        DEBUG_MSG("WARNING: calibration could not access radio A\n");
    }
    if (gw[csn].rf_enable[1] && ((cal_status & 0x04) == 0)) {
        DEBUG_MSG("WARNING: calibration could not access radio B\n");
    }
    if (gw[csn].rf_enable[0] && ((cal_status & 0x08) == 0)) {
        DEBUG_MSG("WARNING: problem in calibration of radio A for image rejection\n");
    }
    if (gw[csn].rf_enable[1] && ((cal_status & 0x10) == 0)) {
        DEBUG_MSG("WARNING: problem in calibration of radio B for image rejection\n");
    }
    if (gw[csn].rf_enable[0] && gw[csn].rf_tx_enable[0] && ((cal_status & 0x20) == 0)) {
        DEBUG_MSG("WARNING: problem in calibration of radio A for TX DC offset\n");
    }
    if (gw[csn].rf_enable[1] && gw[csn].rf_tx_enable[1] && ((cal_status & 0x40) == 0)) {
        DEBUG_MSG("WARNING: problem in calibration of radio B for TX DC offset\n");
    }

    /* Get TX DC offset values */
    for(i=0; i<=7; ++i) {
        lgw_reg_w(csn, LGW_DBG_AGC_MCU_RAM_ADDR, 0xA0+i);
        lgw_reg_r(csn, LGW_DBG_AGC_MCU_RAM_DATA, &read_val);
        gw[csn].cal_offset_a_i[i] = (int8_t)read_val;
        lgw_reg_w(csn, LGW_DBG_AGC_MCU_RAM_ADDR, 0xA8+i);
        lgw_reg_r(csn, LGW_DBG_AGC_MCU_RAM_DATA, &read_val);
        gw[csn].cal_offset_a_q[i] = (int8_t)read_val;
        lgw_reg_w(csn, LGW_DBG_AGC_MCU_RAM_ADDR, 0xB0+i);
        lgw_reg_r(csn, LGW_DBG_AGC_MCU_RAM_DATA, &read_val);
        gw[csn].cal_offset_b_i[i] = (int8_t)read_val;
        lgw_reg_w(csn, LGW_DBG_AGC_MCU_RAM_ADDR, 0xB8+i);
        lgw_reg_r(csn, LGW_DBG_AGC_MCU_RAM_DATA, &read_val);
        gw[csn].cal_offset_b_q[i] = (int8_t)read_val;
    }

    /* load adjusted parameters */
    lgw_constant_adjust(csn);

    /* Sanity check for RX frequency */
    if (gw[csn].rf_rx_freq[0] == 0) {
        fprintf(stderr,"ERROR: wrong configuration, rf_rx_freq[0] is not set\n");
        return LGW_HAL_ERROR;
    }

    /* Freq-to-time-drift calculation */
    x = 4096000000 / (gw[csn].rf_rx_freq[0] >> 1); /* dividend: (4*2048*1000000) >> 1, rescaled to avoid 32b overflow */
    x = ( x > 63 ) ? 63 : x; /* saturation */
    lgw_reg_w(csn, LGW_FREQ_TO_TIME_DRIFT, x); /* default 9 */

    x = 4096000000 / (gw[csn].rf_rx_freq[0] >> 3); /* dividend: (16*2048*1000000) >> 3, rescaled to avoid 32b overflow */
    x = ( x > 63 ) ? 63 : x; /* saturation */
    lgw_reg_w(csn, LGW_MBWSSF_FREQ_TO_TIME_DRIFT, x); /* default 36 */
    ...
    for(i=0; i<LGW_MULTI_NB; ++i) {
        radio_select += (gw[csn].if_rf_chain[i] == 1 ? 1 << i : 0); /* transform bool array into binary word */
    }
    /*
    lgw_reg_w(csn, LGW_RADIO_SELECT, radio_select);
    ...
    */
    ...
    lgw_reg_w(csn, LGW_IF_FREQ_0, IF_HZ_TO_REG(gw[csn].if_freq[0])); /* default -384 */
    lgw_reg_w(csn, LGW_IF_FREQ_1, IF_HZ_TO_REG(gw[csn].if_freq[1])); /* default -128 */
    lgw_reg_w(csn, LGW_IF_FREQ_2, IF_HZ_TO_REG(gw[csn].if_freq[2])); /* default 128 */
    lgw_reg_w(csn, LGW_IF_FREQ_3, IF_HZ_TO_REG(gw[csn].if_freq[3])); /* default 384 */
    lgw_reg_w(csn, LGW_IF_FREQ_4, IF_HZ_TO_REG(gw[csn].if_freq[4])); /* default -384 */
    lgw_reg_w(csn, LGW_IF_FREQ_5, IF_HZ_TO_REG(gw[csn].if_freq[5])); /* default -128 */
    lgw_reg_w(csn, LGW_IF_FREQ_6, IF_HZ_TO_REG(gw[csn].if_freq[6])); /* default 128 */
    lgw_reg_w(csn, LGW_IF_FREQ_7, IF_HZ_TO_REG(gw[csn].if_freq[7])); /* default 384 */

    lgw_reg_w(csn, LGW_CORR0_DETECT_EN, (gw[csn].if_enable[0] == true) ? gw[csn].lora_multi_sfmask[0] : 0); /* default 0 */
    lgw_reg_w(csn, LGW_CORR1_DETECT_EN, (gw[csn].if_enable[1] == true) ? gw[csn].lora_multi_sfmask[1] : 0); /* default 0 */
    lgw_reg_w(csn, LGW_CORR2_DETECT_EN, (gw[csn].if_enable[2] == true) ? gw[csn].lora_multi_sfmask[2] : 0); /* default 0 */
    lgw_reg_w(csn, LGW_CORR3_DETECT_EN, (gw[csn].if_enable[3] == true) ? gw[csn].lora_multi_sfmask[3] : 0); /* default 0 */
    lgw_reg_w(csn, LGW_CORR4_DETECT_EN, (gw[csn].if_enable[4] == true) ? gw[csn].lora_multi_sfmask[4] : 0); /* default 0 */
    lgw_reg_w(csn, LGW_CORR5_DETECT_EN, (gw[csn].if_enable[5] == true) ? gw[csn].lora_multi_sfmask[5] : 0); /* default 0 */
    lgw_reg_w(csn, LGW_CORR6_DETECT_EN, (gw[csn].if_enable[6] == true) ? gw[csn].lora_multi_sfmask[6] : 0); /* default 0 */
    lgw_reg_w(csn, LGW_CORR7_DETECT_EN, (gw[csn].if_enable[7] == true) ? gw[csn].lora_multi_sfmask[7] : 0); /* default 0 */

    lgw_reg_w(csn, LGW_PPM_OFFSET, 0x60); /* as the threshold is 16ms, use 0x60 to enable ppm_offset for SF12 and SF11 @125kHz*/

    lgw_reg_w(csn, LGW_CONCENTRATOR_MODEM_ENABLE, 1); /* default 0 */

    /* configure LoRa 'stand-alone' modem (IF8) */
    lgw_reg_w(csn, LGW_IF_FREQ_8, IF_HZ_TO_REG(gw[csn].if_freq[8])); /* MBWSSF modem (default 0) */
    if (gw[csn].if_enable[8] == true) {
        lgw_reg_w(csn, LGW_MBWSSF_RADIO_SELECT, gw[csn].if_rf_chain[8]);
        switch(gw[csn].lora_rx_bw) {
            case BW_125KHZ: lgw_reg_w(csn, LGW_MBWSSF_MODEM_BW, 0); break;
            case BW_250KHZ: lgw_reg_w(csn, LGW_MBWSSF_MODEM_BW, 1); break;
            case BW_500KHZ: lgw_reg_w(csn, LGW_MBWSSF_MODEM_BW, 2); break;
            default:
                fprintf(stderr,"ERROR: UNEXPECTED VALUE %d IN SWITCH STATEMENT\n", gw[csn].lora_rx_bw);
                return LGW_HAL_ERROR;
        }
        switch(gw[csn].lora_rx_sf) {
            case DR_LORA_SF7: lgw_reg_w(csn, LGW_MBWSSF_RATE_SF, 7); break;
            case DR_LORA_SF8: lgw_reg_w(csn, LGW_MBWSSF_RATE_SF, 8); break;
            case DR_LORA_SF9: lgw_reg_w(csn, LGW_MBWSSF_RATE_SF, 9); break;
            case DR_LORA_SF10: lgw_reg_w(csn, LGW_MBWSSF_RATE_SF, 10); break;
            case DR_LORA_SF11: lgw_reg_w(csn, LGW_MBWSSF_RATE_SF, 11); break;
            case DR_LORA_SF12: lgw_reg_w(csn, LGW_MBWSSF_RATE_SF, 12); break;
            default:
                fprintf(stderr,"ERROR: UNEXPECTED VALUE %d IN SWITCH STATEMENT\n", gw[csn].lora_rx_sf);
                return LGW_HAL_ERROR;
        }
        lgw_reg_w(csn, LGW_MBWSSF_PPM_OFFSET, gw[csn].lora_rx_ppm_offset); /* default 0 */
        lgw_reg_w(csn, LGW_MBWSSF_MODEM_ENABLE, 1); /* default 0 */
    } else {
        lgw_reg_w(csn, LGW_MBWSSF_MODEM_ENABLE, 0);
    }

    /* configure FSK modem (IF9) */
    lgw_reg_w(csn, LGW_IF_FREQ_9, IF_HZ_TO_REG(gw[csn].if_freq[9])); /* FSK modem, default 0 */
    lgw_reg_w(csn, LGW_FSK_PSIZE, fsk_sync_word_size-1);
    lgw_reg_w(csn, LGW_FSK_TX_PSIZE, fsk_sync_word_size-1);
    fsk_sync_word_reg = fsk_sync_word << (8 * (8 - fsk_sync_word_size));
    lgw_reg_w(csn, LGW_FSK_REF_PATTERN_LSB, (uint32_t)(0xFFFFFFFF & fsk_sync_word_reg));
    lgw_reg_w(csn, LGW_FSK_REF_PATTERN_MSB, (uint32_t)(0xFFFFFFFF & (fsk_sync_word_reg >> 32)));
    if (gw[csn].if_enable[9] == true) {
        lgw_reg_w(csn, LGW_FSK_RADIO_SELECT, gw[csn].if_rf_chain[9]);
        lgw_reg_w(csn, LGW_FSK_BR_RATIO, LGW_XTAL_FREQU/gw[csn].fsk_rx_dr); /* setting the dividing ratio for datarate */
        lgw_reg_w(csn, LGW_FSK_CH_BW_EXPO, gw[csn].fsk_rx_bw);
        lgw_reg_w(csn, LGW_FSK_MODEM_ENABLE, 1); /* default 0 */
    } else {
        lgw_reg_w(csn, LGW_FSK_MODEM_ENABLE, 0);
    }

    /* Load firmware */
    load_firmware(csn, MCU_ARB, arb_firmware, MCU_ARB_FW_BYTE);
    load_firmware(csn, MCU_AGC, agc_firmware, MCU_AGC_FW_BYTE);

    /* gives the AGC MCU control over radio, RF front-end and filter gain */
    lgw_reg_w(csn, LGW_FORCE_HOST_RADIO_CTRL, 0);
    lgw_reg_w(csn, LGW_FORCE_HOST_FE_CTRL, 0);
    lgw_reg_w(csn, LGW_FORCE_DEC_FILTER_GAIN, 0);

    /* Get MCUs out of reset */
    lgw_reg_w(csn, LGW_RADIO_SELECT, 0); /* MUST not be = to 1 or 2 at firmware init */
    lgw_reg_w(csn, LGW_MCU_RST_0, 0);
    lgw_reg_w(csn, LGW_MCU_RST_1, 0);
    ...
    lgw_reg_w(csn, LGW_DBG_ARB_MCU_RAM_ADDR, FW_VERSION_ADDR);
    lgw_reg_r(csn, LGW_DBG_ARB_MCU_RAM_DATA, &read_val);
    fw_version = (uint8_t)read_val;
    ...
    for (i = 0; i < txgain_lut[csn].size; ++i) {
        lgw_reg_w(csn, LGW_RADIO_SELECT, AGC_CMD_WAIT); /* start a transaction */
        wait_ms(1);
        load_val = txgain_lut[csn].lut[i].mix_gain + (16 * txgain_lut[csn].lut[i].dac_gain) + (64 * txgain_lut[csn].lut[i].pa_gain);
        lgw_reg_w(csn, LGW_RADIO_SELECT, load_val);
        wait_ms(1);
        lgw_reg_r(csn, LGW_MCU_AGC_STATUS, &read_val);
        if (read_val != (0x30 + i)) {
            fprintf(stderr,"ERROR: AGC FIRMWARE INITIALIZATION FAILURE, STATUS 0x%02X\n", (uint8_t)read_val);
    ...
    if (txgain_lut[csn].size < TX_GAIN_LUT_SIZE_MAX) {
        lgw_reg_w(csn, LGW_RADIO_SELECT, AGC_CMD_WAIT);
        wait_ms(1);
    ...
    lgw_reg_w(csn, LGW_RADIO_SELECT, AGC_CMD_WAIT);
    wait_ms(1);
    lgw_reg_w(csn, LGW_RADIO_SELECT, 3);
    wait_ms(1);
    lgw_reg_r(csn, LGW_MCU_AGC_STATUS, &read_val);
    if (read_val != 0x33) {
        fprintf(stderr,"ERROR: AGC FIRMWARE INITIALIZATION FAILURE, STATUS 0x%02X\n", (uint8_t)read_val);
    ...
    lgw_reg_w(csn, LGW_RADIO_SELECT, AGC_CMD_WAIT);
    wait_ms(1);
    lgw_reg_w(csn, LGW_RADIO_SELECT, 0);
    wait_ms(1);
    lgw_reg_r(csn, LGW_MCU_AGC_STATUS, &read_val);
    if (read_val != 0x30) {
        fprintf(stderr,"ERROR: AGC FIRMWARE INITIALIZATION FAILURE, STATUS 0x%02X\n", (uint8_t)read_val);
    ...
    lgw_reg_w(csn, LGW_RADIO_SELECT, AGC_CMD_WAIT);
    wait_ms(1);
    lgw_reg_w(csn, LGW_RADIO_SELECT, radio_select); /* Load intended value of RADIO_SELECT */
    wait_ms(1);
...
int lgw_receive(uint8_t csn, uint8_t max_pkt, struct lgw_pkt_rx_s *pkt_data)
{
    ...
    /* check if the concentrator is running */
    if (gw[csn].lgw_is_started == false) {
    ...
    for (nb_pkt_fetch = 0; nb_pkt_fetch < max_pkt; ++nb_pkt_fetch) {
        ...
        lgw_reg_rb(csn, LGW_RX_PACKET_DATA_FIFO_NUM_STORED, buff, 5);
        ...
        lgw_reg_rb(csn, LGW_RX_DATA_BUF_DATA, buff, sz+RX_METADATA_NB);
        ...
        p->rf_chain = (uint8_t)gw[csn].if_rf_chain[p->if_chain];
        p->freq_hz = (uint32_t)((int32_t)gw[csn].rf_rx_freq[p->rf_chain] + gw[csn].if_freq[p->if_chain]);
        p->rssi = (float)buff[sz+5] + gw[csn].rf_rssi_offset[p->rf_chain];
        ...
        if ((ifmod == IF_LORA_MULTI) || (ifmod == IF_LORA_STD)) {
            ...
            if (ifmod == IF_LORA_MULTI) {
                ...
                p->bandwidth = gw[csn].lora_rx_bw; /* get the parameter from the config variable */
            }
            sf = (buff[sz+1] >> 4) & 0x0F;
            switch (sf) {
            ...
            p->snr_min = -128.0;
            p->snr_max = -128.0;
            p->bandwidth = gw[csn].fsk_rx_bw;
            p->datarate = gw[csn].fsk_rx_dr;
            p->coderate = CR_UNDEFINED;
            timestamp_correction = ((uint32_t)680000 / gw[csn].fsk_rx_dr) - 20;
        ...
        /* advance packet FIFO */
        lgw_reg_w(csn, LGW_RX_PACKET_DATA_FIFO_NUM_STORED, 0);
    }
...
int lgw_send(uint8_t csn, struct lgw_pkt_tx_s pkt_data)
{
    ...
    /* check input variables */
    if (gw[csn].rf_tx_enable[pkt_data.rf_chain] == false) {
        MSG("ERROR: SELECTED RF_CHAIN IS DISABLED FOR TX ON SELECTED BOARD\n");
        return LGW_HAL_ERROR;
    }
    if (gw[csn].rf_enable[pkt_data.rf_chain] == false) {
    ...
    for (pow_index = txgain_lut[csn].size-1; pow_index > 0; pow_index--) {
        if (txgain_lut[csn].lut[pow_index].rf_power <= pkt_data.rf_power) {
            break;
        }
    }

    /* loading TX imbalance correction */
    target_mix_gain = txgain_lut[csn].lut[pow_index].mix_gain;
    if (pkt_data.rf_chain == 0) { /* use radio A calibration table */
        lgw_reg_w(csn, LGW_TX_OFFSET_I, gw[csn].cal_offset_a_i[target_mix_gain - 8]);
        lgw_reg_w(csn, LGW_TX_OFFSET_Q, gw[csn].cal_offset_a_q[target_mix_gain - 8]);
    } else { /* use radio B calibration table */
        lgw_reg_w(csn, LGW_TX_OFFSET_I, gw[csn].cal_offset_b_i[target_mix_gain - 8]);
        lgw_reg_w(csn, LGW_TX_OFFSET_Q, gw[csn].cal_offset_b_q[target_mix_gain - 8]);
    }

    /* Set digital gain from LUT */
    lgw_reg_w(csn, LGW_TX_GAIN, txgain_lut[csn].lut[pow_index].dig_gain);
    ...
    lgw_reg_w(csn, LGW_TX_DATA_BUF_ADDR, 0);
    lgw_reg_wb(csn, LGW_TX_DATA_BUF_DATA, buff, transfer_size);
    ...
    if (tx_allowed == true) {
        switch(pkt_data.tx_mode) {
            case IMMEDIATE:
                lgw_reg_w(csn, LGW_TX_TRIG_IMMEDIATE, 1);
                break;

            case TIMESTAMPED:
                lgw_reg_w(csn, LGW_TX_TRIG_DELAYED, 1);
                break;

            case ON_GPS:
                lgw_reg_w(csn, LGW_TX_TRIG_GPS, 1);
                break;
...
int lgw_status(uint8_t csn, uint8_t select, uint8_t *code)
{
    ...
    if (select == TX_STATUS) {
        lgw_reg_r(csn, LGW_TX_STATUS, &read_value);
        if (gw[csn].lgw_is_started == false) {
            *code = TX_OFF;
...
int lgw_abort_tx(uint8_t csn)
{
    ...
    i = lgw_reg_w(csn, LGW_TX_TRIG_ALL, 0);
...
int lgw_get_trigcnt(uint8_t csn, uint32_t* trig_cnt_us)
{
    ...
    i = lgw_reg_r(csn, LGW_TIMESTAMP, &val);
